...
Options:
  ...
  -e, --error:            Make actual errors fatal (i.e. terminate execution)
...
local JUnitOutput_MT = { __index = JUnitOutput }
    ...
    function JUnitOutput:addFailure( node )
        io.stdout:write(node:isError() and '# Error: ' or '# Failure: ',
                        node.msg, '\n')
...
----------------------------------------------------------------
    ...
    function M.LuaUnit:addStatus( err )
        ...
        -- "failure" or "error" status below
        local node = self.result.currentNode
        if node:passed() then
            self.result.failureCount = self.result.failureCount + 1
            table.insert( self.result.failures, node )
        end
        if err.status == NodeStatus.ERROR then
            -- always propagate error (even if node already failed before)
            node:error( err.msg, err.trace )
            self.output:addFailure( node )
        elseif node:passed() then -- fail (only) once
    ...
    function M.LuaUnit:endTest()
        ...
        -- "--error" option: runtime errors will lead to program termination
        if self.quitOnError and node:isError() then
            print("\nERROR during LuaUnit test execution:\n" .. node.msg)
            print("LuaUnit ABORTING (as requested by --error option)")
            os.exit(-2)
        end
        self.result.currentNode = nil
    end
